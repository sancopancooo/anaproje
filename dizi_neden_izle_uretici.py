# -*- coding: utf-8 -*-
"""
dizi_neden_izle_uretici.py
--------------------------
Eksik / jenerik "neden izlemeli" maddelerini Groq ile üretir.

AKİŞ (varsayılan):
  1) Sadece JSON havuza yazar  →  json_data/dizi_neden_izle_havuzu.json
...
Çalıştır:
    ...
    python dizi_neden_izle_uretici.py --jenerik

    # Sadece boşlar
    python dizi_neden_izle_uretici.py

    python dizi_neden_izle_uretici.py --jenerik --limit 50
    python dizi_neden_izle_uretici.py --dry-run --jenerik --limit 5

    # Bitince JSON → DB (+ isteğe export)
    python dizi_neden_izle_uretici.py --db-aktar
    python dizi_neden_izle_uretici.py --db-aktar --export
...
DB_DIZI = "katalog.db"
POOL_JSON = os.path.join("json_data", "dizi_neden_izle_havuzu.json")
SAVE_EVERY = 10
SLEEP_SEC = 0.45
...
SYSTEM_PROMPT = """\
Sen uzman bir dizi editörüsün. Sana bir dizi verilecek; o diziye özgü, somut, merak uyandıran
...
3. Her maddenin konusu farklı olsun:
   • 1. madde → dizinin türüne / kurgusuna özgü çengel (korkuysa gerilim dili, komediyse mizah dinamiği vb.)
   • 2. madde → oyuncu / karakter / platform / dönem gibi SOMUT bir kalite (kadroda isim varsa kullan)
...
- Her diziye uyacak jenerik cümleler ve 6 kelimeden kısa maddeler
- Spoiler (final, ölüm, büyük twist açıklama)

YAP:
- Özet ve türe bakarak o diziye özel detay yaz (mekan, meslek, ilişki dinamiği, dönem)
- Korku → korku/gerilim dili; komedi → ilişki/mizah; suç → soruşturma/ahlak gerilimi
- Oyuncu adı kadrodaysa kullan; yoksa uydurma
...
def load_todo(include_generic: bool, pool: dict, limit=None):
    ...
    cur.execute(
        """
        SELECT id, isim, cikis_tarihi, tur, ozet, puan_ortalamasi, durum,
               platformlar, oyuncular_gercek, neden_izlemeli
        FROM diziler
        ORDER BY id
        """
    )
    ...
    for r in cur.fetchall():
        ...
        rows.append({
            "id": r["id"],
            "isim": r["isim"] or "",
            "yil": (r["cikis_tarihi"] or "")[:4],
            "tur": r["tur"] or "",
            "ozet": r["ozet"] or "",
            "puan": r["puan_ortalamasi"],
            "durum": r["durum"] or "",
            "platform": r["platformlar"] or "",
            "oyuncular": r["oyuncular_gercek"] or "",
            "neden_izlemeli": why,
        })
    conn.close()
    if limit:
...
def apply_pool_to_db(export=False):
    ...
    for pid, entry in pool.items():
        ...
        conn.execute(
            "UPDATE diziler SET neden_izlemeli = ? WHERE id = ?",
    ...
    print(f"✅ DB aktarıldı: {updated} dizi (atlanán geçersiz: {skipped})")
...
def run_generate(args):
    print("=" * 70)
    print("📺 DİZİ NEDEN İZLEMELİSİN?  |  Groq → JSON (kaldığı yerden)")
    print("=" * 70)
    ...
    pool = load_pool()
    print(f"💾 Havuzda hazır  : {sum(1 for v in pool.values() if pool_entry_ok(v))} dizi")
    ...
    if not todo:
        ...
        print("   DB'ye aktarmak için: python dizi_neden_izle_uretici.py --db-aktar --export")
        return
    ...
    print("➡️  DB aktarımı: python dizi_neden_izle_uretici.py --db-aktar --export")
    print(f"🕒 {datetime.now().isoformat(timespec='seconds')}")
    print("=" * 70)


def main():
    parser = argparse.ArgumentParser(description="Dizi Neden İzlemelisin? Groq → JSON → DB")
    ...
    if args.db_aktar:
        print("=" * 70)
        print("📦 JSON HAVUZ → VERİTABANI AKTARIMI")
        print("=" * 70)
